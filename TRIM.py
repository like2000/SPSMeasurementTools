...
#############################################################################
# 09-08-2017 Adapted time stamp format and check for injection time,
#         added feedback about files created and adapted to future.
#############################################################################
...
class TrimData(object):
    ...
    def __init__(self, complete_path):
        if complete_path.endswith('.mat'):
            ...
            print('Unknown file extension for TrimData file. Should be '+'.mat')
...
def make_mat_files(trim_data_file, SPSuser, trim_variable=None, trim_data_mat_folder='trim_data'):
    ...
    if not trim_variable:
        trim_variable = list_trim_hist[0][0].replace('/','_').split(' ')[0]

    # check if data is provided using Injection time    ## 09.08.2017 now done while creating unixtimestamp
    ...
    for trim_set_n in xrange(0, len(list_trim_hist[0]), 2):
        ...
        timestamp_str = (list_trim_hist[0][trim_set_n]).split('(')[-1][:-1]
        data_time_vect = np.float_(np.array(time_list))
        data_value_vect = np.float_(np.array(trim_value_list))

        try:
            timestamp_unix = time.mktime(time.strptime(timestamp_str, '%d-%m-%Y %H:%M:%S'))
        except ValueError:
            if trim_set_n == 0:     # Reference setting in trimm data
                timestamp_unix = '0'
            else:
                print('Warning: Timestamp could not be found => set to 0!')
                timestamp_unix = '0'
